...
const CheckinButton = ({ todayRecord, onAction }) => {
    ...
    if (todayRecord?.checkOut) {
        return (
            ...
            rounded-2xl border border-slate-200'>
                <h3 className='text-lg font-bold text-slate-800'>Work Day Completed</h3>
    ...
    return (
        <div className='absolute bottom-4 right-4 flex flex-col z-1'>
            <button className={`w-full max-w-xs flex justify-between items-center gap-8 p-4 rounded-xl bg-linear-to-br
                 text-white ${isCheckedIn ? "from-slate-700 to-slate-900" : "from-fuchsia-600 to-indigo-700"}`}
                onClick={handleAttendence} disabled={loading}>
