  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        {
          !isAdmin && !isDeleted && (
            <button className="bg-linear-to-br from-purple-700 to-fuchsia-900 text-white px-5 py-2.5
             rounded-md hover:from-purple-800 hover:to-fuchsia-950 transition-all duration-300 flex 
             items-center gap-2 w-full sm:w-auto justify-center" onClick={() => setShowModal(true)}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8 ">
            {leaveStats.map((stat) => (
              <div key={stat.label} className=" card card-hover p-5 sm:p-6 flex items-center gap-4 relative overflow-hidden group">
                <div className="absolute left-0 top-0 bottom-0 w-1 rounded-r-full bg-slate-500/70 group-hover:bg-linear-to-br from-slate-600  to-fuchsia-500" />


                <div className='p-3 bg-slate-100 rounded-lg group-hover:bg-indigo-50 transition-colors duration-200 '>

                  <stat.icon className='w-5 h-5 text-slate-600 group-hover:text-fuchsia-700
