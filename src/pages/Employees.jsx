...
const Employees = () => {
  ...
  const [seclectDepart, setSelectDepart] = useState("")
  const [editEmployee, setEditEmployee] = useState(null)
  const [showCreateModal, setShowCreateModal] = useState(false)

  const fetchEmployees = useCallback(async () => {
    setLoading(true)
    setEmployees(dummyEmployeeData.filter((emp) => (seclectDepart ? emp.department === seclectDepart : emp)))
    setTimeout(() => {
      setLoading(false)
    }, 1000)
  }, [seclectDepart])
  ...
  return (
    ...
    <div className="animate-fade-in ">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <button className="btn-primary flex items-center justify-center gap-1.5 w-full sm:w-auto px-3
         py-2 text-sm sm:px-4 sm:py-2.5 sm:text-base"
      ...
      <div className='flex flex-col sm:flex-row gap-3 mb-6 '>
        ...
        <select value={seclectDepart} onChange={(e) => setSelectDepart(e.target.value)} className='max-w-40'>
          <option value="">All Departments</option>
          {
            DEPARTMENTS.map((dept) => {
              return (<option key={dept} value={dept}>
                {dept}
              </option>)
            })
          }
...

      {
        loading ? (
          ...
          (<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5'>
            ...
            rounded-xl border border-dashed border-slate-200 '>No employees found </p>) : (
                filtered.map((emp) => (
          ...
          <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8 animate-fade-in'
            ...
            <div className='flex items-center justify-between p-6 pb-0'>
              ...
              <button className='p-2 rounded-lg hover:bg-slate-100 transition-colors
               text-slate-400 hover:text-slate-600' onClick={() => setShowCreateModal(false)}>
                <X className='w-5 h-5 ' />
              </button>
         ...
         p-4 overflow-y-auto bg-black/40 backdrop-blur-sm' onClick={() => setEditEmployee(null)}>
          ...
          <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8 animate-fade-in'
            ...
            <div className='flex items-center justify-between p-6 pb-0'>
              ...
              <button className='p-2 rounded-lg hover:bg-slate-100 transition-colors
               text-slate-400 hover:text-slate-600' onClick={() => setEditEmployee(null)}>
                <X className='w-5 h-5 ' />
              </button>
